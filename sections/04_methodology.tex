...
\input{tables/benchmark_protocol}
...
\input{tables/comparator_budget}

Table~\ref{tab:comparator-budget} specifies the four classical comparators. RBF-SVC, MLP, and RFF use a TRAIN-fitted standardizer on the original eight feature coordinates; gradient boosting uses those coordinates without standardization. In particular, the classical phase input remains a scalar in radians. They do not replace that coordinate with sine and cosine features. Shared observations therefore do not imply identical preprocessing.
